#[cfg(test)]
mod unit_test;
use crate::{DirectedGraph, BreadthFirstSearch};
use crate::Reader;
...
impl Wordnet{
    ...
    pub fn from_file(synsets_file: &str, hypernyms_file: &str, sep1: char, sep2: char, nb_vertices: usize) -> Self
    {
        let hypernyms = DirectedGraph::from_file(hypernyms_file, sep1, nb_vertices);
        let synsets = Reader::<usize, String>::init(synsets_file.to_string(), sep2).hashmap_from_txt();
    ...
    pub fn is_noun(&self, word: &str) -> bool{
        ...
        self.synset.iter().any(|(k,v)| v.split(" ").collect::<Vec<&str>>().contains(&word))
    }
    pub fn sap_distance(&self, noun_a: &str, noun_b: &str) -> (Option<usize>, Option<Vec<&String>>){
        // shortest ancestor path distance along wiith the path

        let synset_a = self.synset.iter()
        .map(|(k,v)| if v.split(" ").collect::<Vec<&str>>().contains(&noun_a){Some(k)} else {None})
        .collect::<HashSet<_>>();
        let synset_b = self.synset.iter()
        .map(|(k,v)| if v.split(" ").collect::<Vec<&str>>().contains(&noun_b){Some(k)} else {None})
        .collect::<HashSet<_>>();
        ...
        // Some(1)
        if !(synset_a.len()==1 && synset_a.contains(&None)) && !(synset_b.len()==1 && synset_b.contains(&None)){
                        ...
                        if let Some(w) = b{
                            if let Some(ancestor) = sap.ancestor(&self.hypernym_graph, **v, **w){
                                if ancestor.1 + ancestor.2 < min_distance{
                                    ...
                                    path = Some(half_path_one.iter().map(|e| &self.synset[e]).collect::<Vec<&String>>());
                                }
                            }
                        }
                    }
...
impl ShortestAncestralPath{
    ...
    pub fn ancestor(&self, graph: &DirectedGraph, v: usize, w: usize) -> Option<(usize,usize,usize,Vec<usize>,Vec<usize>)>{
